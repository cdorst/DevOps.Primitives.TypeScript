...
namespace DevOps.Primitives.TypeScript.EntityFramework.Services
{
        ...
        where TDbContext : TypeScriptDbContext
    {
        ...
        {
            CacheKey = record => $"{nameof(TypeScript)}.{nameof(ClassDeclaration)}={record.AttributeListCollectionId}:{record.BaseListId}:{record.ConstraintClauseListId}:{record.ConstructorListId}:{record.DocumentationCommentListId}:{record.FieldListId}:{record.IdentifierId}:{record.ImportStatementListId}:{record.MethodListId}:{record.ModifierListId}:{record.NamespaceId}:{record.PropertyListId}:{record.TypeParameterListId}";
        ...
        protected override Action<ClassDeclaration, ClassDeclaration> AssignChanges
            => (existing, given)
